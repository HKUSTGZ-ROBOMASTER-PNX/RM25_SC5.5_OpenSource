...
void bsp_hrtim_init(void)
{
  ...
  LL_HRTIM_DT_SetRisingValue(HRTIM1, LL_HRTIM_TIMER_A, 14);                       // 2.5ns/unit
  LL_HRTIM_DT_SetRisingSign(HRTIM1, LL_HRTIM_TIMER_A, LL_HRTIM_DT_RISING_POSITIVE);
  LL_HRTIM_DT_SetFallingValue(HRTIM1, LL_HRTIM_TIMER_A, 14);
  ...
  LL_HRTIM_DT_SetRisingValue(HRTIM1, LL_HRTIM_TIMER_C, 14); // gan 4
  LL_HRTIM_DT_SetRisingSign(HRTIM1, LL_HRTIM_TIMER_C, LL_HRTIM_DT_RISING_POSITIVE);
  LL_HRTIM_DT_SetFallingValue(HRTIM1, LL_HRTIM_TIMER_C, 14);
  ...
  LL_HRTIM_TIM_SetCounterMode(HRTIM1, LL_HRTIM_TIMER_B, LL_HRTIM_MODE_SINGLESHOT);
  LL_HRTIM_TIM_SetPeriod(HRTIM1, LL_HRTIM_TIMER_B, 0x3E80);
  ...
  LL_HRTIM_TIM_DisableResyncUpdate(HRTIM1, LL_HRTIM_TIMER_B);
	...
	LL_HRTIM_TIM_SetCompare1(HRTIM1, LL_HRTIM_TIMER_MASTER, 4000);
...
void bsp_hrtim_burst_on(void)
{
  ...
  LL_HRTIM_BM_SetPeriod(HRTIM1, 5 - 1);
  LL_HRTIM_BM_SetCompare(HRTIM1, 4 - 1);
  LL_HRTIM_BM_SetMode(HRTIM1, LL_HRTIM_BM_MODE_CONTINOUS);
  LL_HRTIM_BM_SetClockSrc(HRTIM1, LL_HRTIM_BM_CLKSRC_TIMER_B);
...
if (set < 14694.0f)
  {
    buck_duty = set * -0.447675f + 7966.14f;
    boost_duty = 0.9f * PWM_PER_0_5;
  }
  else if (set >= 14694.0f && set < 16000.0f)
  {
    buck_duty = set * -0.45023f + 8003.68f;
    boost_duty = 0.9f * PWM_PER_0_5;
  }
  else if (set >= 16000.0f && set < 17422.0f)
  {
    buck_duty = 0.1f * PWM_PER_0_5;
    boost_duty = -0.413502f * set + 13816.0f;
  }
  else if (set >= 17422.0f)
  {
    buck_duty = 0.1f * PWM_PER_0_5;
    boost_duty = -0.381344 * set + 13843.8f;
  }
  */
